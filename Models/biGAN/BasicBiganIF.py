'''
Basic bigAN net with increasing filter count
'''
...
class BasicBiganIF(BaseBiganModel):
    ...
    def build_generator(self):
        ...
        x = Dense(6*6*128, kernel_constraint=WeightClip(self.w_clip))(z_input)
        x = Reshape([6, 6, 128])(x)
    ...
    def build_encoder(self):
        ...
        x = Dense(512, kernel_constraint=WeightClip(self.w_clip))(x)
        x = LeakyReLU(0.1)(x)
    ...
    def build_discriminator(self):
        ...
        # Latent
        l = Dense(512, kernel_constraint=WeightClip(self.w_clip))(z_input)
        l = LeakyReLU(0.1)(l)
        l = Dense(512, kernel_constraint=WeightClip(self.w_clip))(l)
        l = LeakyReLU(0.1)(l)
        ...
        x = Conv2D(256, (3, 3), padding='same', kernel_constraint=WeightClip(self.w_clip))(x)
        x = BatchNormalization()(x)
        x = LeakyReLU(0.1)(x)
        x = Dropout(rate=self.dropout)(x)
        x = MaxPooling2D((2, 2), padding='same')(x)
        ...
        x = Dense(1024, kernel_constraint=WeightClip(self.w_clip))(x)
        x = LeakyReLU(0.1)(x)
