...
class HighStrideTransposeConvAutoencoder(BaseModel):
    ...
    def __init__(self):
        super().__init__()
        self.name = "HighStrideTransposeConvAutoencoder"
        return
    ...
    def create_net(self, input_shape):
        ...
        x = Conv2D(self.filters, (7, 7), padding='same')(net_input)
        x = PReLU(alpha_initializer=Constant(value=0.25))(x)
        x = MaxPooling2D((2, 2), padding='same')(x)

        x = Conv2D(self.filters, (5, 5), padding='same')(x)
        x = PReLU(alpha_initializer=Constant(value=0.25))(x)
        x = MaxPooling2D((2, 2), padding='same')(x)
        ...
        x = Conv2DTranspose(self.filters, (5,5), strides=(2,2), padding='same')(x)
        x = PReLU(alpha_initializer=Constant(value=0.25))(x)
        ...
        x = Conv2DTranspose(self.filters, (7,7), strides=(2,2), padding='same')(x)
        x = PReLU(alpha_initializer=Constant(value=0.25))(x)
